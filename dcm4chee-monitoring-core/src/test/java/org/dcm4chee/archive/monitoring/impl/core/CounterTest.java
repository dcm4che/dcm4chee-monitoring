...
public class CounterTest {
	...
	@Test
	public void testIfSubContextCounterIncreaseIsPropagated() {
		MonitoringContext serviceCxt = contextProvider.createActiveInstanceContext("test", "service1");
		metricFactory.counter(serviceCxt, Counter.TYPE.DEFAULT);
		

		MonitoringContext subServiceCxt = contextProvider.getActiveInstanceContext().getOrCreateContext("subsystem1");
		Counter subServiceCounter = metricFactory.counter(subServiceCxt, Counter.TYPE.DEFAULT);
		
		subServiceCounter.inc();
		subServiceCounter.inc();
		
		AggregatedReservoirSnapshot snapshot = subServiceCounter.getSnapshot();
		Assert.assertEquals(2, snapshot.getMax());
		
		Counter serviceCounter = metricFactory.counter(serviceCxt, Counter.TYPE.DEFAULT);
		snapshot = serviceCounter.getSnapshot();
		Assert.assertEquals(2, snapshot.getMax());
	}
	
	@Test
	public void testIfSubContextCounterIncreaseAndDecreaseSequenceIsPropagated() {
		MonitoringContext serviceCxt = contextProvider.createActiveInstanceContext("test", "service1");
		metricFactory.counter(serviceCxt, Counter.TYPE.DEFAULT);
		...
		Assert.assertEquals(3, snapshot.getMean(), 2.0);
		
		Counter serviceCounter = metricFactory.counter(serviceCxt, Counter.TYPE.DEFAULT);
		snapshot = serviceCounter.getSnapshot();
		Assert.assertEquals(3, snapshot.getMax());
		Assert.assertEquals(2.0, snapshot.getMean(), 0.0);
		
		subServiceCounter.dec();
		
		snapshot = subServiceCounter.getSnapshot();
		Assert.assertEquals(2, snapshot.getLastValue());
		Assert.assertEquals(2.0, snapshot.getMean(), 0.0);
		...
		snapshot = subServiceCounter.getSnapshot();
        Assert.assertEquals(1, snapshot.getLastValue());
        Assert.assertEquals(3, snapshot.getMax());
        Assert.assertEquals(1.8, snapshot.getMean(), 0.0);
